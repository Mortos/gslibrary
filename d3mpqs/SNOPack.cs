...
namespace d3mpqs
{
    public class SNOPack : MPQPatchChain
    {
        private readonly List<Task> _tasks = new List<Task>();

        public bool LazyLoading = true;
        public bool EnableTasks = true;
        ...
        public SNOPack(int requiredVersion)
            : base(requiredVersion,
            new List<string> { "CoreData.mpq", "ClientData.mpq" },
        ...
        private void LoadCatalog(string fileName, bool useBaseMPQ = false, List<SNOGroup> groupsToLoad = null)
        {
            ...
            if (catalogFile == null)
                throw new Exception(string.Format("Couldn't load catalog file: {0}.", fileName));
        ...
        private void ProcessAsset(Asset asset)
        {
            ...
            if (LazyLoading == false)
            {
                ...
                else
                {
                    ...
                    {
                        throw new Exception(string.Format("Error parsing {0}.\nMessage: {1}\n InnerException:{2}\nStack Trace:{3}", asset.FileName, e.Message, e.InnerException.Message, e.StackTrace), e);
                    }
                }
            }
        }
        ...
        public MpqFile GetFile(string fileName, bool startSearchingFromBaseMPQ = false)
        {
            MpqFile file = null;
